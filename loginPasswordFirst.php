...
<?php
    if(isset($_POST["btn-loginPasswordFirst"]))
    {
        
        //grab the form data
        $userID=$_SESSION["username"];
        $autoPasword =  $_POST["autoPasword"];
        $newPassword =  $_POST["newPassword"];
        $confirmNewPasword =  $_POST["confirmNewPasword"];
        
       
        //check empty field
        if($_SESSION["autopassword"]!=$autoPasword)
        {
            $_SESSION["errMsg"] = "Password is Invalid.Please Check Your Mail!";
            redirect_to("loginPasswordFirst.php");
            $autoPaswordcheck=false;
            return;
        }
        ...
        if(!($autoPaswordcheck)&& !($autoPaswordcheck))
        {
            $sql="UPDATE login SET newpassword = '$newPassword',firstLoginCheck=1 WHERE username = '$userID' ";
            ...
            else
            {
                ...
                header('refresh:2; URL=loginID.php');
            }
        }
    ...
    <div class="login-form">    
        <div class="warning" style="background-color:#f7f7f7;text-align:center;">
            <?php echo success_msg();
                    ...
                    echo err_msg();
                    echo exception_msg(); ?>
            <form action="loginPasswordFirst.php"  method="post" enctype="multipart/form-data">
                <h3 class="text-center"><?php echo $_SESSION["username"] ?></h3>       
                <div class="form-group">
                    <input type="password" id="autoPasword" name="autoPasword" class="form-control" placeholder="Enter Your Password" onblur="validate(this)" required="required">
                </div>
                <h5 class="text-center">Rset Password</h5>
                <div class="form-group">
                ...
                <div class="form-group">
                    <button type="submit" id="btn-login" name="btn-loginPasswordFirst" class="btn btn-primary btn-block">Log In</button>
                </div>
                <div class="clearfix">
